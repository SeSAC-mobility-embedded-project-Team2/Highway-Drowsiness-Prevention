...
uint8_t Compute_Integrated_Risk(uint8_t perclos, float steer_std, float hands_off_sec, float head_delta, float no_op_sec)
{
    ...
    float risk_eye   = Fuzzy_Trapezoid((float)perclos, 50.0f, 70.0f);
    float risk_steer = Fuzzy_Trapezoid(steer_std, 20.0f, 40.0f);
    float risk_hands = Fuzzy_Trapezoid(hands_off_sec, 2.0f, 5.0f);
    float risk_head  = Fuzzy_Trapezoid(ABS(head_delta), 6.0f, 25.0f);
    float risk_noop  = Fuzzy_Trapezoid(no_op_sec, 3.0f, 15.0f) * 0.8f; // 최대 80점(Warning) 제한

    // [Step 2] Rule Evaluation (눈부심 방지)
    // 눈만 위험하고(>0.8), 나머지는 아주 멀쩡하면(<0.1) -> 눈부심(Glare)으로 간주
    if (risk_eye > 0.8f)
    {
        if (risk_steer < 0.2f && risk_hands < 0.1f && risk_head < 0.1f)
        {
        	if(no_op_sec < 9.0f)
        	{
        		risk_eye = 0.3f;
