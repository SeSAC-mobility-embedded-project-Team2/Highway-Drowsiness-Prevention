...
def wrap_packet_uart(payload):
    """ [기능] UART 전송용 래핑 (STX + Payload + Checksum + ETX) """
    header = b'\x02'  # STX
    ...
    while True:
        ...
        if a != -1 and b != -1:
            ...
            if (curr_time - last_tx_time) >= TX_CYCLE:
                ...
                print(f"[UART] {payload_hex} | Alive:{alive_cnt} Detect:{face_detect_flag} PERCLOS:{perclos:.1f}%")
