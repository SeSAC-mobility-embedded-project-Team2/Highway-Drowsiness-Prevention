...
def main():
    ...
    while True:
        ...
        if a != -1 and b != -1:
            ...
            # 2. PERCLOS 계산 (0~100)
            # 여기서는 계산은 하지만 전송은 안 함 (디버깅 위해)
            real_perclos = 0.0
            if len(eye_history) > 0:
                real_perclos = (sum(eye_history) / len(eye_history)) * 100
            ...
            if (curr_time - last_tx_time) >= TX_CYCLE:
                ...
                # ==========================================================
                # [DEBUG] 디버깅을 위해 PERCLOS 값을 100으로 고정합니다!
                # ==========================================================
                forced_perclos = 100.0  # <--- [여기서 강제로 100 설정]

                # 패킷 생성 (강제값 사용)
                payload_8byte = create_dbc_payload(forced_perclos, eye_state_raw, face_detect_flag, alive_cnt, vision_err_flag)
                ...
                # 터미널 로그 출력 (강제값인지 확인용)
                # 100(10진수) = 64(16진수) 이므로 [UART 8B] 64 ... 로 떠야 정상
                payload_hex = ' '.join(f'{b:02X}' for b in payload_8byte)
                print(f"[UART 8B] {payload_hex} | Alive:{alive_cnt} Detect:{face_detect_flag} PERCLOS(Forced):{forced_perclos:.1f}%")
                
                # 타이머 갱신
                last_tx_time = curr_time

            # --- [화면 시각화] ---
            # 화면에는 '실제' 계산된 값을 보여줌 (비교를 위해)
            fps = 1 / (curr_time - prev_time) if (curr_time - prev_time) > 0 else 0
            prev_time = curr_time

            cv2.putText(image, f"FPS: {int(fps)}", (30, 460), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (150, 150, 150), 2)
            
            # [디버깅 알림] 화면에 빨간 글씨로 전송값이 100임을 표시
            cv2.putText(image, "DEBUG MODE: SENDING 100%", (200, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

            # 실제 계산값 표시 (전송값 아님)
            cv2.putText(image, f"Real PERCLOS: {real_perclos:.1f}%", (30, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
